      ...
      <body>
        ...
        <h3>Linki</h3>
          <xsl:copy-of select="$przycisk"/>
           <xsl:apply-templates select="projekcik/pasja/cena"/><br/>
          <xsl:apply-templates select="projekcik/pasja/strona[@part='linki']"/>
...
<xsl:template match="nazwy">
 ...
 <xsl:template name="nazwa">
  ...
  <xsl:template match="autor">
    <div class="autor">
      <xsl:text>Utworzone przez: </xsl:text>
      <xsl:for-each select="*">
        <xsl:value-of select="."/>
      </xsl:for-each>
...
<xsl:template match="kostka">
    ...
    <xsl:for-each select="ocz">
      <xsl:sort select="oczka" order="descending"/>
      Oczko numer: <xsl:number value="position()" format="I"/>
      <br/>
